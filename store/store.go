...
import (
	...
	"github.com/Akihira77/fullstack-go-htmx/types"
)
...
type Store interface {
	CreateCar(c *types.Car) (*types.Car, error)
...
func (s *Storage) GetCars() ([]types.Car, error) {
	rows, err := s.db.Query("SELECT id, brand, make, model, year, imageURL, createdAt FROM cars")
	if err != nil {
		return nil, err
	}
...
func (s *Storage) FindCarsByNameMakeOrBrand(search string) ([]types.Car, error) {
	rows, err := s.db.Query("SELECT id, brand, make, model, year, imageURL, createdAt FROM cars WHERE brand LIKE ? OR model LIKE ? OR make LIKE ?", "%"+search+"%", "%"+search+"%", "%"+search+"%")
	if err != nil {
		return nil, err
	}
